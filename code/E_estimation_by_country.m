...
hold(ax3,'on');
T_ord=readtable("inventory_emissions_country.xlsx",'Sheet',2,'VariableNamingRule','preserve');
y=1:45;x=T_ord{:,10};
colors=readtable("inventory_emissions_country.xlsx",'Sheet',3,'VariableNamingRule','preserve');
...
lon=-14.875:0.25:54.875;lat=73.875:-0.25:34.125;
country_raster=imread('Europe_country_0p25deg.tif');
...
hold on;
data=shaperead('ne_10m_admin_0_countries.shp','UseGeoCoords',true);
data = data(contains({data.CONTINENT}, 'Europe'));
data=struct('Geometry',{data.Geometry},'BoundingBox',{data.BoundingBox},...
    'Lon',{data.Lon},'Lat',{data.Lat},'GEOUNIT',{data.GEOUNIT},...
    'NOTE_ADM0',{data.NOTE_ADM0},'HOMEPART',{data.HOMEPART},'ADM0_A3',{data.ADM0_A3});
S=data;
